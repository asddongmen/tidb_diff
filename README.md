...
1. **数据库连接关闭等待（已优化）**
   ...
   - **解决方案**：
     - 工具已添加连接关闭超时机制（5秒），会自动处理此问题
     - 在关闭连接前会先释放空闲连接（`SetMaxIdleConns(0)` 和 `SetMaxOpenConns(0)`），加快关闭速度
     - 超时后会记录警告日志但不阻塞主程序退出
   - **预防措施**：
     - 显式设置 `query_timeout_seconds`，避免查询时间过长
     - 检查数据库慢查询日志，确认是否有查询执行时间过长
     - 监控数据库连接数，确认是否连接池耗尽
     - 如果看到 "关闭源库连接超时，强制退出" 或 "关闭目标库连接超时，强制退出" 的警告，说明有查询仍在运行，建议检查 `query_timeout_seconds` 配置

2. **查询超时设置过长或 Context 未及时取消（已优化）**
   - **原因**：
     - 如果 `query_timeout_seconds = 0`（使用默认 10 分钟），对于超大表可能不够
     - Context 取消时机不当，可能导致资源未及时释放
   - **解决方案**：
     - 根据最大表的大小显式设置 `query_timeout_seconds`（**强烈建议**）
     - 工具已优化 Context 取消时机，查询完成后立即取消，不再等待函数结束
     ```ini
...
5. **Goroutine 泄漏（已优化）**
   - **原因**：某些 goroutine 未正确退出，导致程序无法结束
   - **解决方案**：工具已优化连接关闭逻辑，确保所有 goroutine 都能正确退出
   - **检查方法**：如果程序仍然阻塞，可以使用以下方法诊断：
     ```bash
